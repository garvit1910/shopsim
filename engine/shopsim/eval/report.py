...
def render_markdown(report: dict) -> str:
    ...
    A("Generated by `make eval` (`python -m shopsim.eval report`). Every number "
      "here is reproduced from scratch by that command, with ONE stated "
      "exception: the `before` column of 7.2 is a citation, not a "
      "reproduction — it measures an engine that no longer exists, and its "
      "provenance is recorded in `results/calibration.json` under "
      "`before_source`.\n")

    laws = report.get("laws") or []
    missing = report.get("laws_not_run") or []
    total = len(laws) + len(missing)
    passed = sum(1 for x in laws if x["passed"])
    headline = f"## Face-validity suite — {passed}/{len(laws)} measured laws passing"
    if missing:
        headline += f", {len(missing)} of {total} not measured"
    A(headline + "\n")
    ...
    if missing:
        A("Not run in this pass (named rather than omitted): "
          + ", ".join(f"`{m['id']}`{' **(never-drop)**' if m.get('never_drop') else ''}"
    ...
    realised = cal.get("realised") or {}
    if realised and rows:
        n = cal.get("decisions") or {}
        A("### Model-implied vs realised\n")
        A("The table above is the **model-implied** mean of each gate "
          "probability over the decisions that actually happened. The realised "
          "counts from the same run are below, and they do **not** simply "
          f"agree: at {n.get('page_decisions', '?')} page decisions the "
          "realised column is thin, but its misses are one-sided — every "
          "realised rate sits above its model value, not scattered around it. "
          "The model estimator is primary because it is the one readable at "
          "demo scale, which is a choice of estimator and not a licence to "
          "call the gap agreement. `calibration.md` works the standard errors "
          "through.\n")
        A("| metric | band | model-implied | realised | realised in band |")
        A("|---|---|---|---|---|")
        after = cal.get("after") or {}
        bands = {r["metric"]: r["band"] for r in rows}
        alias = {"p_click": "ctr"}
        for metric in sorted(bands):
            rv = realised.get(alias.get(metric, metric))
            band = bands[metric]
            ok = rv is not None and band[0] <= rv <= band[1]
            mv = after.get(metric)
            A(f"| `{metric}` | {band[0]:.3f}–{band[1]:.3f} | "
              f"{'—' if mv is None else format(mv, '.4f')} | "
              f"{'—' if rv is None else format(rv, '.4f')} | "
              f"{'—' if rv is None else ('✓' if ok else '✗')} |")
        A(f"\nCreative decisions: {n.get('creative_decisions', '?')} · "
          f"page decisions: {n.get('page_decisions', '?')}.\n")

    cbd = report.get("ctr_by_day") or {}
    if cbd:
        A("## CTR by simulated day\n")
        A(f"Mean **{cbd['mean']:.4f}**, range {cbd['min']:.4f}–{cbd['max']:.4f}, "
          f"against the {cbd['band'][0]:.3f}–{cbd['band'][1]:.3f} band "
          f"({cbd['source']}).")
        if cbd["days_out_of_band"]:
            A(f"Days outside the band: {cbd['days_out_of_band']} "
              f"({len(cbd['days_out_of_band'])} of {len(cbd['series'])}).")
        A(f"\n*{cbd['note']}*\n")
